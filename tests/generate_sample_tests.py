...
fn {name}() {{
    ...
    let result = context.compile(&contents, &Options {{ optimize: {optimize} }});
...
code = (FILE_HEADER + 
       "\n".join([FUNCTION.format(sample_path=f, name=Path(f).stem, optimize='false') for f in files]) + 
       "\n".join([FUNCTION.format(sample_path=f, name=Path(f).stem + '_optimized', optimize='true') for f in files]))
